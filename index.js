...
function parseLeadId(responseText) {
  // Response format: "SUCCESS: add_lead - 393757456818 - newlead 9"
...
app.post("/push-leads", async (req, res) => {
  ...
  if (!apiUser || !apiPass) {
    return res.status(500).json({ error: "Vicidial credentials not configured" });
  }

  const { leads, list_id, campaign_id } = req.body;

  if (!leads || !Array.isArray(leads) || !list_id) {
    return res.status(400).json({ error: "Missing required fields: leads, list_id" });
  }
  ...
  for (const lead of leads) {
    try {
      ...
      console.log(`Pushing lead ${lead.lead_ref} to Vicidial...`);

      const response = await fetch(url);
      const text = await response.text();
      console.log(`Vicidial response for ${lead.lead_ref}: ${text}`);
      ...
      console.error(`Failed to push lead ${lead.lead_ref}:`, err);
      results.push({
        lead_ref: lead.lead_ref,
        success: false,
...
app.post("/create-list", async (req, res) => {
  ...
  if (!list_id || !list_name || !campaign_id) {
    return res.status(400).json({ error: "Missing required fields: list_id, list_name, campaign_id" });
  }
